...
from database import create_database, select_n_last_messages, add_message
...
from spkit import text_to_speech
...
def start(message):
    bot.send_message(message.from_user.id, "Привет! Отправь мне голосовое сообщение или текст, и я тебе отвечу!", reply_markup=create_keyboard([ '/help', "/debug"]))
    create_database()
# обрабатываем команду /help
@bot.message_handler(commands=['help'])
def help(message):
    bot.send_message(message.from_user.id, "Чтобы приступить к общению, отправь мне голосовое сообщение или текст", reply_markup=create_keyboard([ '/start', "/debug"]))

# обрабатываем команду /debug - отправляем файл с логами
@bot.message_handler(commands=['debug'])
def debug(message):
    with open("logs.txt", "rb") as f:
        bot.send_document(message.chat.id, f, reply_markup=create_keyboard([ '/start', "/help"]))
...
def handle_voice(message: telebot.types.Message):
    try:
     ...
     #   # Обработка голосового сообщения
          ...
          status_stt, stt_text = text_to_speech(file)
          if not status_stt:
